...
function Profile() {
  ...
  const handleDeleteItem = async (itemId, itemType) => {
    if (!window.confirm("Are you sure you want to remove this report from the grid?")) return;
    ...
    try {
      ...
      setUserData(prev => {
        ...
        return {
          ...prev,
          items: updatedItems,
          lostCount: updatedItems.filter(i => i.type?.toLowerCase() === 'lost').length,
          foundCount: updatedItems.filter(i => i.type?.toLowerCase() === 'found').length,
        };
      });
  ...
  return (
    <div style={styles.container}>
      <style>{`
        ...
        .delete-btn:hover { color: #ff4d4d !important; transform: scale(1.2); transition: 0.2s; }
      `}</style>
            ...
            <AnimatePresence>
              {filteredItems.map((item) => (
                <motion.div 
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className={`item-card ${item.type === 'lost' ? "neon-border-lost" : "neon-border-found"}`} 
                ...
                >
                  ...
                  <div style={{...styles.typeBadge, color: item.type === 'lost' ? '#ff4d4d' : '#4ade80', marginLeft: '15px'}}>{item.type.toUpperCase()}</div>
                </motion.div>
              ))}
            </AnimatePresence>
...
const styles = {
  ...
  deleteIconButton: { background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '16px', opacity: 0.6, marginLeft: '10px' }
};
